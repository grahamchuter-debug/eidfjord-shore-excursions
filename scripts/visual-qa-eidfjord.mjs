/**
 * Temporary Eidfjord visual QA harness.
 */
...
const pages = [
  ...
  { path: "/excursions/voringsfossen-waterfall", label: "voringsfossen" },
  { path: "/eidfjord-port-guide", label: "port-guide" },
  { path: "/one-day-in-eidfjord", label: "one-day" },
  { path: "/is-eidfjord-worth-visiting", label: "worth-visiting" },
  { path: "/best-time-to-visit-eidfjord", label: "best-time" },
    ...
    for (const route of pages) {
      ...
      if (
        metrics.emailLinks.some((h) =>
          (h || "").includes("hello@eidfjordshoreexcursions.com"),
        )
      ) {
        note(
          "FAIL",
      ...
      if (metrics.sibling && !route.path.includes("eidfjord-port-guide")) {
