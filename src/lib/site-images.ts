export const siteImages = {
  hero:
    "https://upload.wikimedia.org/wikipedia/commons/b/b6/Voringsfossen_waterfall_at_Eidfjord%2C_Norway.jpg",
  voringsfossen:
    "https://upload.wikimedia.org/wikipedia/commons/f/fb/V%C3%B8ringfossen.jpg",
  voringsfossenViewpoint:
    "https://upload.wikimedia.org/wikipedia/commons/b/b6/Voringsfossen_waterfall_at_Eidfjord%2C_Norway.jpg",
  harbour:
    "https://upload.wikimedia.org/wikipedia/commons/c/cd/Eidfjord_-_panoramio.jpg",
  village:
    "https://upload.wikimedia.org/wikipedia/commons/9/92/Eidfjord.jpg",
  hardangerfjord:
    "https://upload.wikimedia.org/wikipedia/commons/c/c2/Hardangerfjord.jpg",
  hardangervidda:
    "https://upload.wikimedia.org/wikipedia/commons/f/fd/Hardangervidda_20150723_170645.jpg",
  mabodalen:
    "https://upload.wikimedia.org/wikipedia/commons/4/41/M%C3%A5b%C3%B8dalen_-_Eidfjord%2C_Norway_2021-07-25.jpg",
  eioRiver:
    "https://upload.wikimedia.org/wikipedia/commons/e/e7/Norge_M%C3%A5b%C3%B8dalen._Ruten_Eidfjord_-_Fossli_-_Geilo.jpg",
  fossliViewpoint:
    "https://upload.wikimedia.org/wikipedia/commons/a/af/M%C3%A5b%C3%B8dalen%2C_2011_August.jpg",
  portGuide:
    "https://upload.wikimedia.org/wikipedia/commons/c/cd/Eidfjord_-_panoramio.jpg",
  worthVisiting:
    "https://upload.wikimedia.org/wikipedia/commons/f/fb/V%C3%B8ringfossen.jpg",
  bestOfTour:
    "https://upload.wikimedia.org/wikipedia/commons/b/b6/Voringsfossen_waterfall_at_Eidfjord%2C_Norway.jpg",
  privateBestOfTour:
    "https://upload.wikimedia.org/wikipedia/commons/c/c2/Hardangerfjord.jpg",
  voringsfossenTour:
    "https://upload.wikimedia.org/wikipedia/commons/f/fb/V%C3%B8ringfossen.jpg",
  hardangerviddaTour:
    "https://upload.wikimedia.org/wikipedia/commons/f/fd/Hardangervidda_20150723_170645.jpg",
  flamPortCard:
    "https://upload.wikimedia.org/wikipedia/commons/5/56/Fl%C3%A5m_fr%C3%A5_cruiseskip_ved_kai.jpg",
  bergenPortCard:
    "https://upload.wikimedia.org/wikipedia/commons/f/fe/Harbour_Bergen_Norway_2009_5.jpg",
  stavangerPortCard:
    "https://upload.wikimedia.org/wikipedia/commons/3/3a/Stavanger_havn.jpg",
  alesundPortCard:
    "https://upload.wikimedia.org/wikipedia/commons/c/c7/%C3%85lesund_Hafen_lub_2025-07-28_img04.jpg",
  geirangerPortCard:
    "https://upload.wikimedia.org/wikipedia/commons/1/10/Flydalsjuvet_Geiranger_Geirangerfjorden.jpg",
  oldenPortCard:
    "https://upload.wikimedia.org/wikipedia/commons/c/ca/Briksdalsbreen_Glacier_-Norway.jpg",
} as const;

export const imageAlts = {
  hero: "Vøringsfossen waterfall plunging into Måbødalen valley near Eidfjord on Hardangerfjord, Norway",
  voringsfossen:
    "Vøringsfossen waterfall with dramatic canyon walls in Måbødalen near Eidfjord cruise port",
  voringsfossenViewpoint:
    "Vøringsfossen waterfall viewed from the Fossli Hotel viewpoint area above Måbødalen",
  harbour:
    "Eidfjord harbour and waterfront on Hardangerfjord — Eidfjord cruise port, Norway",
  village:
    "Eidfjord village nestled on Hardangerfjord with mountain scenery behind the cruise-friendly harbour",
  hardangerfjord:
    "Hardangerfjord fjord scenery with steep mountains and calm water near Eidfjord, Norway",
  hardangervidda:
    "Hardangervidda mountain plateau with open tundra and distant peaks above Eidfjord",
  mabodalen:
    "Måbødalen valley road and steep mountain walls leading toward Vøringsfossen from Eidfjord",
  eioRiver:
    "Scenic Eio River and Måbødalen valley route between Eidfjord and Vøringsfossen viewpoints",
  fossliViewpoint:
    "Fossli Hotel viewpoint area overlooking Måbødalen valley on the road to Vøringsfossen",
  portGuide:
    "Eidfjord cruise port waterfront with Hardangerfjord mountains — port guide for cruise passengers",
  worthVisiting:
    "Vøringsfossen waterfall and Hardanger scenery — why Eidfjord is worth visiting on a Norway cruise",
  bestOfTourCard:
    "Best of Eidfjord sightseeing tour with Vøringsfossen waterfall and Hardangervidda Nature Centre",
  privateBestOfTourCard:
    "Private Best of Eidfjord sightseeing with flexible pace and Hardangerfjord mountain scenery",
  voringsfossenTourCard:
    "Vøringsfossen Waterfall Viewpoint Tour with dramatic Norway waterfall scenery near Eidfjord",
  hardangerviddaTourCard:
    "Hardangervidda Nature Centre and Måbødalen valley on a scenic Eidfjord shore excursion",
  flamPortCard:
    "Flam harbour with cruise ship at the pier — Flam Shore Excursions",
  bergenPortCard: "Bergen harbour and waterfront — Bergen Shore Excursions",
  stavangerPortCard:
    "Stavanger harbour with cruise-friendly waterfront — Stavanger Shore Excursions",
  alesundPortCard:
    "Alesund harbour with cruise-friendly waterfront — Alesund Shore Excursions",
  geirangerPortCard:
    "Geirangerfjord viewpoint with cruise ship — Geiranger Shore Excursions",
  oldenPortCard:
    "Briksdal Glacier and Nordfjord scenery — Olden Shore Excursions",
} as const;
