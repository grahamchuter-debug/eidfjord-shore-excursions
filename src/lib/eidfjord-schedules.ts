import schedulePayload from "@/data/eidfjord-cruise-schedules.generated.json";

export type EidfjordScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type EidfjordScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: EidfjordScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const eidfjordScheduleIntegrity = payload.integrity;
export const eidfjordScheduleMeta = {
  port: payload.port,
...
function toEntry(row: EidfjordScheduleRow): EidfjordScheduleEntry {
  return {
...
export function getAllEidfjordRows(): EidfjordScheduleRow[] {
  return payload.rows;
}

export function getEidfjordEntries(): EidfjordScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getEidfjordEntriesForMonthKey(monthKey: string): EidfjordScheduleEntry[] {
  return payload.rows
...
export function getEidfjordMonthKeysWithCalls(): string[] {
  return [...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7)))].sort();
}

export function getEidfjordMonthSummaries() {
